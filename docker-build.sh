...
if [ "$1" == "" ]; then
    git archive --format=tar --worktree-attributes --prefix=screenshot-api-server/ -o latest.tar HEAD
    #docker rmi ym9315/yang:book-js-api-${IMAGE_VERSION}
    docker build -f Dockerfile -t registry.cn-hangzhou.aliyuncs.com/consume/book-js:latest .
    rm -rf latest.tar
    docker push registry.cn-hangzhou.aliyuncs.com/consume/book-js:latest
fi
