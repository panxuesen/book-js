...
const renderImage = function (req, res, next) {
    ...
    browserHelper.loadPage({
        ...
        imgPath = helper.makeImgFileInfo()
        let rect = {
            width: postParam.width,
            height: postParam.height,
            top: postParam.top ? postParam.top: 0,
            left: postParam.left ? postParam.left: 0,
        }

        let imageData = await browserHelper.screenshotDOMElement(page, element, imgPath, rect);
        if (imageData) {
            // notify.send(helper.successMsg({image: 'data:image/png;base64,' + imageData}))
            notify.send(helper.successMsg({image: imgPath.relatePath}))
        } else {
